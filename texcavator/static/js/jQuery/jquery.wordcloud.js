...
(function ($) {
	...
	$.fn.wordCloud = function (options) {
		...
		var settings = {
			...
			wordList: [],
			rotateRatio: 0.0,	// 0.1: default, 0.0: no vertical words
			clearCanvas: true,
			fillBox: false
		};
						...
						function (gxy) {
							if (canFitText(gxy[0], gxy[1], gw, gh)) {
								if (mu !== 1 || rotate) {
									...
									wordMap.push({word: word, 
												  x: Math.floor(gxy[0]*g + (gw*g - w)/2),
												  y: Math.floor(gxy[1]*g + (gh*g - h)/2),
												  w: w, h: h, size: w*h});
								} else {
		...
		return this.each(function() {
			...
			$el.click(wordMap, function(event) {
				var wordMap = event.data.sort(function(a,b) { return a.size-b.size; });
				
				var hit = function(item, x, y) {
					if(x < item.x) return false;
					if(x > (item.x + item.w)) return false;
					if(y < item.y) return false;
					if(y > (item.y + item.h)) return false;
					return true;
				}
				for(var index = 0; index < wordMap.length; index++) {
					if(hit(wordMap[index], event.offsetX, event.offsetY)) {
						console.log("Clicked '" + wordMap[index].word + "' @ " + event.offsetX + "," + event.offsetY);
						event.type = "wordcloudclick";
						event.word = wordMap[index].word;
						$el.trigger(event);
						return;
					}
				}
				console.log("Clicked no word @ " + event.offsetX + "," + event.offsetY);				
			});
